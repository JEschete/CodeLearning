...
\begin{lstlisting}
	...
	// A more modern, but non-standard, alternative is #pragma once
	#pragma once
	class Account {
		// ...
	};
...
When a class has pointer member variables, the default copy constructor performs a shallow copy (it just copies the pointer's value, not what it points to). This can lead to problems, like two objects pointing to the same memory. A deep copy involves creating a new copy of the pointed-to data. You often need to implement your own copy constructor to perform a deep copy.
...
\begin{lstlisting}
	...
	void add_movie(std::vector<Movie> &movies, std::string name, std::string rating, int watched) {
