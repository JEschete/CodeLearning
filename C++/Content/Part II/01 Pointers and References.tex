...
\end{lstlisting}
\begin{remark}
	An uninitialized pointer contains a garbage address. Dereferencing it is undefined behavior and typically crashes the program. Always initialize your pointers — to \texttt{nullptr} if they don't yet point to a valid object, or directly to an address.
\end{remark}
...
\begin{lstlisting}
	...
	int const *p2 {&some_int}; // same as p1 — cannot modify *p2

	// 2. Constant pointer: the pointer itself cannot be reassigned.
	int *const p3 {&some_int}; // p3 always points to some_int
	*p3 = 99; // OK — can modify the value
	// p3 = nullptr; // ERROR — cannot reseat the pointer

	// 3. Constant pointer to a constant: neither the pointer nor the data can change.
	const int *const p4 {&some_int};
\end{lstlisting}

\begin{remark}
	Read pointer declarations right-to-left: \texttt{int *const p3} is ``\texttt{p3} is a const pointer to int''. \texttt{const int *p1} is ``\texttt{p1} is a pointer to const int''. The position of \texttt{const} relative to the \texttt{*} is what distinguishes them.
\end{remark}
...
You can return a pointer from a function, but you must be careful not to return a pointer to a local variable, as it will be destroyed when the function exits. It's usually used for returning dynamically allocated memory.
\begin{lstlisting}
...
\begin{remark}
	\textbf{Never} return a pointer to a local variable. The local variable is destroyed when the function returns, leaving the caller with a dangling pointer:
	\begin{lstlisting}
		int* bad_function() {
			int local {42};
	...
	\end{lstlisting}
	When a function must return dynamically allocated memory, prefer returning a \texttt{std::unique\_ptr} instead of a raw pointer — that way ownership is explicit and the caller cannot forget to free it.
\end{remark}

\subsection{Potential Pointer Pitfalls}
\begin{itemize}
	\item \textbf{Dangling pointers:} Pointers that point to memory that has already been freed.
...
\begin{example}[Common Pointer Bugs]{\leavevmode}
\begin{lstlisting}
	// 1. Dangling pointer
	int *p = new int {42};
	delete p;
	std::cout << *p << std::endl; // UNDEFINED BEHAVIOR — p points to freed memory
	p = nullptr;                  // Fix: null it out after deleting

	// 2. Null pointer dereference
	int *q {nullptr};
	// std::cout << *q; // CRASH — always check before dereferencing
	if (q != nullptr)
		std::cout << *q << std::endl;
	...
	void leaky() {
		int *r = new int {10};
		// Forgot delete r; — memory is leaked every time leaky() is called
	}
\end{lstlisting}
\end{example}

\begin{remark}
	All three of these pitfalls are eliminated by using smart pointers (\texttt{std::unique\_ptr}, \texttt{std::shared\_ptr}), covered in Part IV. Prefer smart pointers over raw \texttt{new}/\texttt{delete} in modern C++.
\end{remark}
...
\subsubsection{Pointer vs.\ Reference: When to Use Each}
\begin{itemize}
	\item Use a \textbf{reference} when: the parameter or return value must never be \texttt{null}, you don't need to reseat it, and you want clean, dereference-free syntax. References are the default choice for pass-by-reference parameters.
	\item Use a \textbf{pointer} when: the value might legitimately be \texttt{null} (optional parameter), you need to reseat it to point to different objects, or you are doing pointer arithmetic / managing arrays.
\end{itemize}
In modern C++, raw pointers are increasingly replaced by references, smart pointers, or higher-level containers. If you find yourself choosing between a raw pointer and a reference, prefer the reference.
