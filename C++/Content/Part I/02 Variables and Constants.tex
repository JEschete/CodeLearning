...
\begin{remark}
	...
	Signed integer overflow is even more dangerous — it is \textbf{undefined behavior} in C++. The compiler is allowed to assume it never happens and may generate incorrect code as a result. Use a wider type (\texttt{long long}) or check bounds before the operation if overflow is possible.
\end{remark}
...
\begin{lstlisting}
	#include <cmath>   // Required for std::abs with floating-point
...
\begin{remark}
	\texttt{static\_cast<int>(ch)} explicitly converts a \texttt{char} to an \texttt{int} so that \texttt{cout} prints the numeric value rather than the character. Type casting is covered in detail in the Statements and Operators section.
\end{remark}
...
\begin{lstlisting}[numbers=none]
	...
	Size of long: 8 byte(s)    // Linux/macOS; Windows gives 4!
...
\begin{remark}
	\texttt{long} is the most platform-dependent integer type. On Linux and macOS, \texttt{long} is 8 bytes on 64-bit systems. On Windows (including with MSVC and MinGW), \texttt{long} is \textbf{4 bytes}—the same size as \texttt{int}. If you need a guaranteed 64-bit integer, use \texttt{long long} or \texttt{int64\_t} from \texttt{<cstdint>}.
\end{remark}
...
\begin{lstlisting}
	...
	std::cout << sizeof(age) << std::endl;       // 4 (int)
	std::cout << sizeof(salary) << std::endl;    // 8 (double)
	std::cout << sizeof(age + salary) << std::endl;  // 8 (double)
\end{lstlisting}

\begin{remark}
	In the expression \texttt{age + salary}, the \texttt{int} is automatically promoted to \texttt{double} before the addition so the types match — this is called \textbf{implicit type promotion}. The result is a \texttt{double}, which is why \texttt{sizeof} returns 8. We will cover type conversions fully in the Statements and Operators section.
\end{remark}
...
\begin{example}[Exercise 4: \texttt{const}, \texttt{constexpr}, and \texttt{auto}]{\leavevmode}

	The following code has several issues relating to constants and type deduction. Identify and fix them:

	\begin{enumerate}
		\item \texttt{pi} should be a compile-time constant (it never changes and its value is known at compile time) — use \texttt{constexpr}
		\item \texttt{greeting} is declared as a modifiable variable but should never change — add the appropriate qualifier
		\item The type of \texttt{result} is obvious from context — replace the explicit type with \texttt{auto}
		\item \texttt{user\_name} uses \texttt{\#define} — replace it with a proper typed constant
	\end{enumerate}
