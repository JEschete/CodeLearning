...
\end{lstlisting}
\begin{remark}
	The \texttt{break} statement is crucial. Without it, execution ``falls through'' to the next case — all following cases run until a \texttt{break} or the end of the \texttt{switch} is reached. Accidental fall-through is a common bug. When fall-through is intentional, annotate it with the \texttt{[[fallthrough]]} attribute (C++17) to silence compiler warnings and signal intent:
	\begin{lstlisting}
		case 1:
		case 2:
...
\begin{remark}
	The \texttt{switch} statement only works with \textbf{integer or character} values. You cannot switch on a \texttt{std::string}. A common beginner mistake is writing \texttt{switch(my\_string)} — this will not compile. Use \texttt{if-else if} chains for string comparisons.
\end{remark}
...
\begin{remark}
	\texttt{int score : scores} copies each element. For large or non-trivial types (like \texttt{std::string}), prefer a \textbf{const reference} to avoid unnecessary copies:
	\begin{lstlisting}
		std::vector<std::string> names {"Alice", "Bob", "Charlie"};
		for (const std::string &name : names) {   // no copy
			std::cout << name << "\n";
		}
		// Or with auto:
		for (const auto &name : names) { ... }
	\end{lstlisting}
	Use \texttt{auto \&name} (without \texttt{const}) only if you intend to modify the element in-place.
\end{remark}
...
\begin{lstlisting}
	// This loop will run forever (commented out intentionally)
	// while (true) {
	//     std::cout << "Looping... ";
	// }
\end{lstlisting}
...
\begin{remark}
	Intentional infinite loops with \texttt{while (true)} are a valid and common pattern — for example, a game loop or a server waiting for connections. In these cases, you use \texttt{break} or \texttt{return} to exit cleanly. The danger is an \textit{unintentional} infinite loop caused by a logic error in the condition (e.g., forgetting to increment a counter).
\end{remark}
