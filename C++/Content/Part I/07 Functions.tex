...
\begin{remark}
	Default arguments must appear at the \textbf{end} of the parameter list. You cannot skip a middle parameter — omitted arguments are filled in right-to-left.
	\begin{lstlisting}
		void f(int a, int b = 2, int c = 3); // OK
		void g(int a = 1, int b, int c = 3); // ERROR - b has no default
	\end{lstlisting}
\end{remark}
...
\begin{remark}
	The functions above take \texttt{std::string} \textbf{by value}, which makes a copy each time they are called. For read-only string (and other large object) parameters, prefer passing by \textbf{const reference} to avoid the copy:
	\begin{lstlisting}
		// By value (makes a copy - avoidable overhead)
		void print_greeting(std::string name, int age);

		// By const reference (no copy - preferred for read-only use)
		void print_greeting(const std::string &name, int age);
	\end{lstlisting}
	Pass by value is fine for small, cheap-to-copy types like \texttt{int}, \texttt{double}, \texttt{char}, and \texttt{bool}.
\end{remark}
...
For very short functions, you can suggest to the compiler that it perform \texttt{inline} expansion. This copies the function's code directly into the calling location, which can avoid the overhead of a function call. This is only a hint to the compiler — modern compilers routinely inline small functions automatically.
\begin{lstlisting}
	inline int add(int a, int b) { return a + b; }
\end{lstlisting}

\begin{remark}
	\texttt{inline} has a second, more important role: it allows a function to be \textbf{defined in a header file} without causing a linker error. Normally, if a header is included in multiple \texttt{.cpp} files, a non-inline function defined in it would be defined multiple times, causing a ``multiple definition'' linker error. \texttt{inline} tells the linker to expect multiple identical definitions and merge them. This is why small utility functions and template functions defined in headers are typically marked \texttt{inline}.
\end{remark}

\subsection{Recursive Functions}
A recursive function is a function that calls itself, either directly or indirectly. A recursive function must have a \textbf{base case} (a condition that stops the recursion) to avoid infinite recursion.
\begin{lstlisting}
	// factorial function
...
\begin{remark}
	Each recursive call pushes a new frame onto the \textbf{call stack}. If the recursion is too deep (or the base case is never reached), the stack runs out of space and the program crashes with a \textbf{stack overflow}. For example, calling \texttt{factorial(-1)} with the above code would recurse forever. Always ensure your base case is reachable for all valid inputs.
\end{remark}
