...
\begin{example}[Common \texttt{<cctype>} Functions]{\leavevmode}
	\begin{lstlisting}
		#include <iostream>
		#include <cctype>
...
\begin{remark}
	Many C-style string functions are unsafe because they do not perform bounds checking. \texttt{strcpy} and \texttt{strcat} can easily lead to \textbf{buffer overflows} if the destination array is not large enough — a classic security vulnerability. Modern C++ code should prefer \texttt{std::string}. The exercises below practice the C-style API so you can read and understand legacy code, but new code should use \texttt{std::string}.
\end{remark}
...
\subsubsection{String Conversion Functions}

Converting between strings and numeric types is a common need. The \texttt{<string>} header provides:
...
\begin{remark}
	\texttt{std::stoi} and \texttt{std::stod} throw \texttt{std::invalid\_argument} if the string is not a valid number, and \texttt{std::out\_of\_range} if the value is too large for the type. Exception handling is covered in a later section.
\end{remark}
...
\begin{lstlisting}
	...
	int main() {
		...
		for (char c : secret_message) {
			// .find() returns std::string::npos (a large sentinel value)
			// if the character is not found
			size_t position = alphabet.find(c);
			if (position != std::string::npos) {
				encrypted_message += key[position];
			} else {
				encrypted_message += c; // non-alphabet chars pass through
			}
		}
