...
\begin{remark}
	\textbf{Which smart pointer to choose?}
	\begin{itemize}
		\item \texttt{std::unique\_ptr} — default choice. Zero overhead, exclusive ownership, transfers via \texttt{std::move}.
		\item \texttt{std::shared\_ptr} — when multiple owners genuinely need to share a resource and the last one should free it.
		\item \texttt{std::weak\_ptr} — only to observe a \texttt{shared\_ptr}-managed object without extending its lifetime, typically to break a cycle.
	\end{itemize}
	If you are unsure, start with \texttt{unique\_ptr}. Upgrade to \texttt{shared\_ptr} only when you actually need shared ownership.
\end{remark}
...
\begin{itemize}
    ...
    \item Cannot be dereferenced directly. You must convert it to a \texttt{shared\_ptr} first using \texttt{.lock()}.
\end{itemize}
\begin{lstlisting}
...
public:
	// std::shared_ptr<A> a_ptr; // This would create a circular reference —
	                             // A owns B, B owns A, neither is ever freed
	std::weak_ptr<A> a_ptr;     // weak_ptr breaks the cycle
};
\end{lstlisting}

To actually use a \texttt{weak\_ptr}, call \texttt{.lock()} to obtain a temporary \texttt{shared\_ptr}:
\begin{lstlisting}
std::shared_ptr<int> shared = std::make_shared<int>(42);
std::weak_ptr<int>   weak   = shared;

if (auto locked = weak.lock()) { // returns nullptr if object was already freed
